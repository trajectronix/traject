...
inline constexpr ::std::strong_ordering operator<=>(basic_os_c_str_with_known_size<char_type> a, basic_os_c_str_with_known_size<char_type> b) noexcept {
	...
	for (; (a.ptr != a_last) && (b.ptr != b_last);) {
		if (auto i{*a.ptr <=> *b.ptr}; i != ::std::strong_ordering::equal)
			return i;
		++a.ptr;
		++b.ptr;
	}
...
inline consteval auto expand_all_parameters_and_check(::fast_io::freestanding::array<parameter const *, N> const &punsort) noexcept {
	...
	for (::std::size_t i{}; i < N; i++) {
		...
		for (::std::size_t j{}; j < punsort[i]->alias.len; j++) {
			res[res_pos++] = {punsort[i]->alias.base[j], punsort[i]};
		}
	}
...
inline consteval ::std::size_t calculate_max_para_size(::fast_io::freestanding::array<all_parameter, N> const &punsort) noexcept {
	::std::size_t max_size{};
	for (::std::size_t i{}; i < N; i++) 
		max_size = ::std::max(max_size, punsort[i].str.size());
	return max_size;
}
	...
	for (int i{hash_size_base}; i < sizet_d10; i++) {
		...
		for (auto &j : ord) {
			...
			for (::std::size_t k{}; k < j_str_size; k++) {
				ptr[k] = static_cast<::std::byte>(j.str.ptr[k]);
			}
			crc32c.reset();
...
template <::std::size_t hash_table_size, ::std::size_t conflict_size>
inline constexpr parameter const *find_from_hash_table(parameters_hash_table<hash_table_size, conflict_size> const &ht, ::fast_io::basic_os_c_str_with_known_size<char> str) noexcept {
	::fast_io::crc32c_context crc32c{};
...
#endif
	{
		...
		for (::std::size_t k{}; k < str_size; k++) {
			ptr[k] = static_cast<::std::byte>(str.ptr[k]);
		}
